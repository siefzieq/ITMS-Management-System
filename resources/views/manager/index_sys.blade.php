        ...
        @foreach($devInfos as $d)
            <div class="col-md-4">
                <div class="card" style="width: 100%; border-radius: 20px; box-shadow: 0 4px 8px rgba(0, 0, 0, 0.1);">
                    <div class="card-body">
                        <h4 class="card-title">{{$d->projects->project_title??'None'}}</h4>
                        <p>Platform : {{$d->platform}}</p>
                        <a class="btn text-white float-right" style="background-color:#0461AA;" href="{{ route('devInfo.show', $d->id) }}">More info</a>
                    </div>
                </div>
                <br>
            </div>
